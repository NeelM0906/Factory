/**
 * Live harness smoke for Codex.
 *
 * Skipped by default — runs only when `AUTOSTACK_LIVE_HARNESS_SMOKE=1` is set
 * AND the `codex` CLI is installed and authenticated. The skip is visible
...
describe.skipIf(reason != null)("codex live smoke", () => {
  it("drives a trivial objective to completion on a disposable repository", async () => {
    ...
    try {
      ...
      const availability = await probeCodexAvailability({ executable: "codex" });
      expect(availability.installed).toBe(true);
      expect(availability.authenticated).toBe(true);

      // Create harness with the real CLI in app-server mode
      ...
      const invocation = AgentInvocationRequestSchema.parse({
        schemaVersion: 1,
        idempotencyKey: "live-smoke:codex:start",
