...
describe("authoritative runner policy", () => {
  ...
  it("authorizes expired recovery operations only when every persisted identity agrees", async () => {
    ...
    const environment = EnvironmentAuthorizationSchema.parse({
      ...
      digest: await digestEnvironmentAuthorization({ ...environmentWithoutDigest, digest: DIGEST })
    });
      ...
      await decideEnvironmentDisposal({
        authority: {
          ...
          resolveTerminalRunEvidence: async () => ({
            workspaceId: WORKSPACE_ID,
            runId: RUN_ID,
            evidence: { status: "cancelled", terminalEventSequence: 7, terminalEventDigest: DIGEST }
          })
        },
        authenticatedWorkspaceId: WORKSPACE_ID,
        request: disposalRequest,
