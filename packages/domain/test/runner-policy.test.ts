...
// Crypto-digest bound: these cases recompute real SHA-256 scope, spec, and authorization digests
// across whole persisted envelopes, and measure 0.1-0.4s on an unconstrained dev machine. CI runs
// `pnpm test:coverage` on a 2-vCPU runner with V8 coverage instrumentation while every workspace
// package tests in parallel, which stretches them past the 5s default. Applied per case so the
// package-wide default keeps protecting every other test.
const SLOW_CASE = { timeout: 15_000 } as const;

describe("runner policy authority", () => {
  it(
    "issues frozen envelopes from persisted records and rechecks them at start",
    SLOW_CASE,
    async () => {
      const records: {
        run?: Run;
      ...
      const authority: ExecutionPolicyAuthority = {
        ...
        resolveApproval: async (id) =>
          id === PLAN_APPROVAL_ID ? records.plan : records.permission,
      ...
      const issueWithCommand = async (requestedCommand: unknown) => {
        const scoped = {
          ...commandScope,
          commandDigest: await digestCommandSpec(requestedCommand)
        };
        records.permission = await approval(
          PERMISSION_APPROVAL_ID,
          "permission",
  ...
  it(
    "accepts offset authorization instants and rejects invalid or future authorization use",
    SLOW_CASE,
    async () => {
      ...
      expect(
        await decideCommandStart({
          authority: {
            ...authority,
            resolveCommandAuthorization: async () => commandAuthorization
          },
          authenticatedWorkspaceId: WORKSPACE_ID,
          request,
          now: offsetNow
