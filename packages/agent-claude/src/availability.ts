/**
 * Claude Code availability probe.
 *
 * Runs `claude auth status` under a bounded timeout, reads its exit status,
 * and passes any surfaced output through redaction. Never reads `~/.claude` (D-6).
 *
...
/**
 * Probe Claude Code availability by running `<executable> auth status`.
 *
 ...
 */
export const probeClaudeAvailability = async (
  ...
  try {
    ...
    if (message.includes("ENOENT")) {
      return buildResult(false, false, "Claude Code executable not found.", checkedAt);
    }
