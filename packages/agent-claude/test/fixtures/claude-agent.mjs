...
/**
 * Fixture Claude Code CLI: replays a transcript file's frames over real stdio.
 *
 * Usage: node claude-agent.mjs <transcript-name>
 *
 ...
 *
 * This script speaks the Claude Code stream-json protocol — it does NOT
 * speak ACP. It ignores its own argv beyond the transcript name, so the
 * harness can pass whatever flags it wants.
 */
...
if (!transcriptName) {
  process.stderr.write("Usage: claude-agent.mjs <transcript-name>\n");
  process.exit(2);
}
