...
import { runReviewStation } from "../../src/stations/review-station.js";
...
/** The implement station's agent session id — the review must use a different one. */
...
const buildVerificationReport = async () => {
...
/**
 * Builds the full event history for a run in "reviewing" status. The run has gone through:
 * triage -> plan -> plan_approval -> provisioning -> implementing -> verifying -> reviewing.
 */
const reviewingRun = async (): Promise<readonly StoredDomainEvent[]> => {
  ...
  const planDocument = await buildPlanDocument();
  const verificationReport = await buildVerificationReport();
  return [
    stored(
      "run.created",
      {
...
/**
 * A ReviewReport in structured harness output: the review session returns this as its verdict.
 */
const approvedReviewReport = async () => {
  const planDocument = await buildPlanDocument();
  const verificationReport = await buildVerificationReport();
  const verificationReportDigest = await digestVerificationReport(verificationReport);
  return {
    schemaVersion: 1,
    workspaceId: WORKSPACE_ID,
    workItemId: WORK_ITEM_ID,
    ...
    summary: "All acceptance criteria met. Code quality is satisfactory.",
    findings: [
      {
        findingRef: "finding-1",
        severity: "info" as const,
        summary: "Minor formatting suggestion.",
        evidenceDigest: digestOf("f")
      }
    ],
    producedAt: NOW,
    producedBy: {
...
const changesRequestedReviewReport = async () => {
  const report = await approvedReviewReport();
  return {
    ...report,
    verdict: "changes_requested" as const,
    summary: "Critical bug found in error handling.",
    findings: [
      {
        findingRef: "finding-1",
        severity: "high" as const,
        summary: "Missing null check on parser input.",
        evidenceDigest: digestOf("f"),
        location: {
          path: "packages/parser/src/reader.ts",
          startLine: 42,
          endLine: 45
        }
      }
...
const defaultHarnessScript = async (): Promise<FakeHarnessScript> => {
  const report = await approvedReviewReport();
  return [
...
const changesRequestedHarnessScript = async (): Promise<FakeHarnessScript> => {
  const report = await changesRequestedReviewReport();
  return [
    { kind: "emit", event: { type: "started" } },
    {
      kind: "emit",
...
const dependencies = async (
...
): Promise<StationDependencies> => ({
  now: () => NOW,
  random: () => 0.5,
  signal: new AbortController().signal,
  ids: createIdFactory(() => UUID),
  harness: createFakeAgentHarness({
    script: await defaultHarnessScript(),
    now: () => NOW,
  ...
  readRunEvents: async () => reviewingRun(),
  workspaceId: WORKSPACE_ID,
  actor: ACTOR,
  ...overrides
});
...
const runReview = async (
  ...
  const injected = await dependencies(overrides);
  return runReviewStation(
...
describe("review station approved verdict", () => {
  it("transitions to awaiting_publish_approval and enqueues nothing when approved", async () => {
    ...
    expect(transition?.payload.to).toBe("awaiting_publish_approval");

    // Approved review requests publish approval, it does not enqueue a job (D2).
    expect(result.jobs).toHaveLength(0);
  });

  it("emits ReviewEvidence with different implementation and reviewer sessions", async () => {
    const result = await runReview();
    ...
    expect(evidence.stage).toBe("isolated_review");
    expect(evidence.verdict).toBe("approved");

    // D7: implementation and reviewer sessions must differ.
    const implementation = evidence.implementation as Record<string, unknown>;
    const reviewer = evidence.reviewer as Record<string, unknown>;
    expect(implementation.agentSessionId).not.toBe(reviewer.agentSessionId);
    expect(implementation.environmentId).not.toBe(reviewer.environmentId);
  });
});

describe("review station changes_requested verdict", () => {
  it("routes back to implement with attempt + 1 on changes_requested", async () => {
    const result = await runReview({
      harness: createFakeAgentHarness({
        script: await changesRequestedHarnessScript(),
        now: () => NOW,
        providerSessionRef: () => "provider-session"
      })
    });
    ...
    const transition = eventsOf(result).find((e) => e.type === "run.transitioned");
    expect(transition?.payload.to).toBe("implementing");

    expect(result.jobs).toHaveLength(1);
    expect(result.jobs[0]!.handler).toBe("pipeline.implement");
    expect(result.jobs[0]!.payload.attempt).toBe(2);
  });
});

describe("review station cancellation", () => {
  it("abandons on an aborted signal", async () => {
    const controller = new AbortController();
    controller.abort();

    await expect(
      runReview({ signal: controller.signal })
