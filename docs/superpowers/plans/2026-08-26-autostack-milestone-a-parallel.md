...
- [ ] **Approval-actor verification at composition (S5 merge-review boundary note, 2026-08-31):** `SlackApprovalAction.userId` and GitHub's `authorId` are DATA from the provider payload, never a grant — both adapters document this per §14.1. The obligation that the acting user is verified against approval policy (eligibleApproverIds) and that `evidenceDigest` is compared to the stored approval belongs to S4/composition and MUST be wired at I1, or "downstream's job" becomes nobody's job. Also: `parseApprovalAction` is typed synchronous but Slack binding resolution is realistically async — composition needs a pre-loaded synchronous binding cache or a route-signature change; decide at I1, not mid-wiring.
- [ ] **Control-plane run-action routes (orchestrator ruling at S1 close, 2026-08-31):** the clarification-answer route (`POST /v1/runs/:runId/clarifications/:clarificationRef/answer`, schemas at `105db8e`, server-derived idempotency, S1's plan holds a ready route sketch) and the approvals-list route honouring the `status: "all"` wildcard (`db926c3` — "all" = no filter; S6's mock shows the shape) belong to I1, which owns the composition root — NOT to any Wave-1 stream. S1's T14 sketch is the implementation starting point. Also carried to I1: S1's T11 taxonomy question (a contracts-conformant port cannot deliver credential-shaped content, so `model_output_unsafe` is defense-in-depth and port-boundary refusals surface as internal error — decide whether the taxonomy wants a dedicated code at composition) and S1's proposed non-blocking steer-fold conformance obligation for `packages/domain`'s suite.
- [ ] **S5 composition deliverables (from its stream report, 2026-08-31):** (a) `IngressQueue`'s durable store — spec §13.2 compliance rides on it (SQLite-backed, drained by intake); (b) the two-provider `DeliveryIntegrationPort` facade over the github/slack factories lives in the composition root, not in either package; (c) Slack inbound needs `resolveBindingByChannel` wired at composition (outbound uses `resolveBindingByRef`); (d) the ingress→intake converter per the requester-mapping pin above.
