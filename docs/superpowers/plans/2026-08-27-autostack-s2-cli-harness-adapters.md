...
- **Every package this stream owns sets `fileParallelism: false` and `testTimeout: 15_000`**, matching `packages/runner-local`. Both settings are inherited from that package's measured evidence rather than guessed: a suite that spawns real children is hostage to whole-machine spawn latency, which is not proportional to CPU load, so contention shows up as rare extreme outliers (one measured case went 818ms → 5022ms, 6.1×) rather than as an even stretch. This stream has that profile and then some — a real fixture child per conformance subject, two runners, and the pause and permission behaviours repeated twenty times. A budget calibrated on an idle machine turns those repeats into a lottery whose failure lands on whichever case lost rather than on whatever is actually wrong.
...
The full suite is run under **bounded concurrency**. The form above is CI's own idiom and is correct — do not "fix" it by inserting `--`. That single change is the whole trap, and the expansion pnpm prints shows why:

```text
pnpm test --concurrency=2       ->  turbo run test --concurrency=2       # turbo gets the flag
pnpm test -- --concurrency=2    ->  turbo run test -- --concurrency=2    # turbo forwards it to vitest
```

The root script is `turbo run test`, and turbo passes everything after `--` through to the underlying task. So the bare form works, `pnpm exec turbo run test --concurrency=2` works, and only the `--` form is broken. It fails loudly rather than running silently unbounded, but the point is not to reach for it.
