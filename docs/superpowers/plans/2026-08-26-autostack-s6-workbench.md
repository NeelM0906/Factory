...
- **Conversation** renders `AgentSessionStreamEvent[]` ordered by `sequence`, groups `message` / `thought_summary` / `tool_call` distinctly, renders `permission_requested` with its `summary` and `permissionRef` (CORRECTED at Task 5a step 2: the original prose said "with its options", but the session-stream event carries no `options` field — the option set lives on the separate `AgentPermissionRequestSchema` used by the permission-response channel; any later task rendering `permission_requested` — 5b/6/10a — must not assume options either) and `permission_resolved` with the chosen `selectedOptionId`, and renders `interrupted` as a distinct, non-color-only state from `failed` (contract audit item 5). Load-bearing test: an out-of-order input array still renders in `sequence` order.
...
| Queue depth           | `stage.queued` not yet terminal (succeeded/failed), per stage, by `jobId` — CORRECTED at Task 9a step 2: the original "queued minus leased" shorthand contradicted this task's own pinned rule (an unmatched `stage.leased` contributes to depth); subtraction cancels a crashed lease to zero, silently vanishing outstanding work. Depth reads as "work not yet done", not "work waiting for a worker" |
