...
- **Observed-red is not enough — the red must come from the defect the test claims to reject.** "I broke it and the test went red" is satisfied by deleting the component entirely, which proves nothing about discrimination. **Every guard test names, in a comment, WHICH wrong implementation it rejects, and its red evidence must come from _that_ defect.** The discovering case (S6): a `"Not recorded"` fallback test could not tell `value || fallback` from `value === undefined ? fallback : value`, because `undefined` is falsy and both pass — only companion assertions on `0` and `""` (falsy but _defined_) gave the guard teeth. **Corollary:** when a guard concerns a sentinel or a default, the vectors must include values that are _adjacent but distinguishable_ — falsy-but-defined, present-but-different, trimmed-vs-untrimmed — not just the obvious absent case.
