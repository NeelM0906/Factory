...
- **D-16 — commit per step, not per task.** Each red/green step ends in its own commit, and **committing a failing test alone is legitimate**: `test(scope): add failing X` is a far better recovery point than an uncommitted tree. Briefs **enumerate the per-step commit messages**, so a recovering session reads `git log` instead of reconstructing intent. The task-level gate (full package verification) moves to the final step rather than guarding every commit.
  - **Consequence for this stream's single-committer model.** Implementers were barred from all mutating git precisely to stop concurrent implementers racing the index. That justification expired: since Task 3 this stream has run **one implementer at a time**. Under D-16 the sole running implementer commits its own steps; the lead reviews after, and review fixes land as their own commits. **If two implementers are ever dispatched concurrently again, the no-mutating-git rule returns and the lead commits** — the two rules trade off, and the deciding variable is concurrency, not preference. An implementer that reports only on completion is the stall fragility wearing a different hat: its work is undurable until someone else acts.
- **Observed-red is not enough — the red must come from the defect the test claims to reject.** "I broke it and the test went red" is satisfied by deleting the component entirely, which proves nothing about discrimination. **Every guard test names, in a comment, WHICH wrong implementation it rejects, and its red evidence must come from _that_ defect.** The discovering case (S6): a `"Not recorded"` fallback test could not tell `value || fallback` from `value === undefined ? fallback : value`, because `undefined` is falsy and both pass — only companion assertions on `0` and `""` (falsy but _defined_) gave the guard teeth. **The standing question (S6, final form — carry this, not the case list).** Every vacuous guard found across the fleet shares **one shape: the environment supplies a default that happens to satisfy the assertion.** So ask of every guard: **"what does the environment return when the feature is ABSENT, and does that value pass?"** If it passes, the guard is decorative. This generalizes past the known instances (jsdom CSS-blindness, falsy-undefined, click-not-moving-focus), which is why the question travels and the examples do not.
